...
OBSERVER_PATH = ROOT / "scripts" / "b0" / "exposure_observer.py"
DIAGNOSTIC_RUNNER_PATH = (
    ROOT / "scripts" / "b0" / "reference" / "run_b0_exposure_diagnostic.executed.py"
)
ARCHIVED_TEST_PATH = ROOT / "tests" / "reference" / "b0_exposure_observer_v1.py"
...
_EXPECTED_SHA256 = {
    OBSERVER_PATH: "dd582ab3011d3077c1ad1f63b95f26708813bf0628dcdee4bebda4c054a68d57",
    DIAGNOSTIC_RUNNER_PATH: "dfe718d6ea28fc188072f596f935665c8299ad7148b20bef11b4d4a0482c5128",
    ARCHIVED_TEST_PATH: "2864fc2ec1eea2d8524ce46c9abd16d6609044be055d035a3a2e47f42207ac6d",
}
for _path, _expected_hash in _EXPECTED_SHA256.items():
    if _sha256(_path) != _expected_hash:
        raise RuntimeError(f"published B0 provenance changed: {_path.name}")
...
if _test_method_asts(Path(__file__)) != _test_method_asts(ARCHIVED_TEST_PATH):
    raise RuntimeError("active B0 observer test methods differ from validated source")

SPEC = importlib.util.spec_from_file_location(
    "b0_exposure_observer_under_test", OBSERVER_PATH
)
...
# The executed diagnostic runner is archival: importing it would load TraCI and
# its historical local-evidence topology.  Extract only the four pure helpers
# exercised by the validated tests, preserving their exact ASTs.
_HELPERS = {
    "visit_entry_period",
    "select_event_period_visit_pair",
    "restricted_lane_compliance",
    "determine_diagnostic_status",
}
_runner_tree = ast.parse(
    DIAGNOSTIC_RUNNER_PATH.read_text(encoding="utf-8"),
    filename=str(DIAGNOSTIC_RUNNER_PATH),
)
_helper_nodes = [
    node
...
if {node.name for node in _helper_nodes} != _HELPERS:
    raise RuntimeError("validated diagnostic helper set is incomplete")
RUNNER = types.ModuleType("b0_exposure_diagnostic_pure_helpers")
RUNNER.__dict__.update(
    {"Any": Any, "Iterable": Iterable, "Mapping": Mapping, "math": math}
)
exec(
    compile(
        ast.Module(body=_helper_nodes, type_ignores=[]),
        str(DIAGNOSTIC_RUNNER_PATH),
        "exec",
    ),
    RUNNER.__dict__,
)
